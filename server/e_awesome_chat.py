...
from ar_models.LawyerRAG.lawyer import Lawyer
from ar_models.TourismRAG.tourism import Tourism
lawyer_llm = Lawyer()
tourism_llm = Tourism()
...
def chat_huggingface(messages, api_key, api_type, api_endpoint, return_planning = False, return_results = False):
    ...
    ## Lawyer Assistant
    pinput = "قم بتحليل هذا الإدخال وصنفه اذا كان يتعلق بأي من القانون أو القضاء أو العقوبات أوالأنظمة فقم بإرجاع 'legal-advice' بدون أي كلمات إضافية, اما اذا كانت غير ذلك قم بإرجاع [failed]. الآن سأدخل [{{input}}]"
    ...
    if response.strip("'") == "legal-advice":
        record_case(success=True, **{"input": input, "task": "legal-advice", "reason": "legal-question task", "op": "legal-question-response"})
        response = lawyer_llm.inference(input)
        print(response)
        if "لا أعرف" in response:
    ...
    if tasks[0]["task"] == "tourism":
        ...
        response = tourism_llm.inference(input)
        return {"message": response}
...
def cli():
    ...
    keywords = ["من أنت", "من انت", "من هو لبيب", "صنعك", "عرفني على لبيب", "اخبرني عن نفسك", "أخبرني عن نفسك", "من لبيب", "صناعتك"]
    while True:
...
def evaluate(parse_results, ground_truth):
    all_f1_scores = []
    all_edit_distances = []

    for result, gt in zip(parse_results, ground_truth):
        ...
        try:
            ...
            print(f"Skipping invalid ground truth: {gt}")
            continue

        # Calculate F1 Score
        pred_tasks = [task['task'] for task in result]
        gt_tasks = [task['task'] for task in gt]

        common_tasks = set(pred_tasks).intersection(set(gt_tasks))
        y_true = [1 if task in common_tasks else 0 for task in gt_tasks]
        y_pred = [1 if task in common_tasks else 0 for task in pred_tasks]

        f1 = f1_score(y_true, y_pred, average='macro')
        all_f1_scores.append(f1)

        # Calculate Edit Distance
        edit_dist = calculate_edit_distance(pred_tasks, gt_tasks)
        all_edit_distances.append(edit_dist)

    return {
        "average_f1_score": np.mean(all_f1_scores),
        "average_edit_distance": np.mean(all_edit_distances)
    }

def evaluation():
    # Generate parse results using the parse_task function
    with open('demos/eval_parse_task.json', 'r', encoding='utf-8') as file:
        dataset = json.load(file)
    parse_results = []
    ground_truth = [task['content'] for task in dataset if task['role'] == 'assistant']
    for task in dataset:
        if task['role'] == 'user':
            context = []
            input = task['content']
            parse_result = parse_task(context, input, API_KEY, API_TYPE, API_ENDPOINT)
            parse_results.append(json.loads(parse_result))
            print(f"parse_result: {parse_result}")  # Debug print

    print("end loop")
    # Evaluate the results
    evaluation_results = evaluate(parse_results, ground_truth)
